    ...
    <header className="w-full sticky top-0 z-30 bg-background/80 dark:bg-background-dark/80 backdrop-blur border-b border-border dark:border-border-dark">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center gap-3">
        ...
        <div className="flex items-center min-w-0">
          <Link to="/" className="text-lg font-bold text-heading dark:text-heading-dark hover:opacity-90 transition-opacity select-none">Tracker</Link>
        </div>
  ...
  <div className="flex-1 hidden md:flex justify-center">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="relative" ref={ddRef}>
            <button
              className="relative inline-flex items-center justify-center h-10 w-10 rounded-md hover:bg-surface dark:hover:bg-surface-dark"
              onClick={() => setOpen((s) => !s)}
              aria-haspopup="menu"
              aria-expanded={open}
            ...
            >
              <Bell size={18} />
              <span className="absolute -top-0.5 -right-0.5 bg-error text-white text-[10px] leading-none px-1.5 py-0.5 rounded-full">3</span>
            </button>
            {open && (
              <div className="absolute right-0 mt-2 w-72 bg-surface dark:bg-surface-dark border border-border dark:border-border-dark rounded-lg shadow-lg overflow-hidden">
                <div className="px-3 py-2 text-sm border-b border-border dark:border-border-dark">Notifications</div>
                <ul className="max-h-64 overflow-auto">
                  <li className="px-3 py-2 text-sm hover:bg-background/60 dark:hover:bg-background-dark/60 cursor-pointer">You have a new follower</li>
                  <li className="px-3 py-2 text-sm hover:bg-background/60 dark:hover:bg-background-dark/60 cursor-pointer">Habit: Read — 1 new comment</li>
                  <li className="px-3 py-2 text-sm hover:bg-background/60 dark:hover:bg-background-dark/60 cursor-pointer">Weekly summary is ready</li>
                </ul>
              </div>
            )}
          </div>

          <Link to="/profile" className="inline-flex items-center gap-2 h-10 px-3 rounded-md bg-primary text-white hover:bg-primary-dark" title="Profile">
